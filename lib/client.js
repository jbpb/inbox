 ...
 */
IMAPClient.prototype.storeMessage = function(message, options, callback){
    if(typeof options == "function" && !callback){
        callback = options;
        options = undefined;
    }
    ...
    var flags = [].concat(options.flags || []);
    ...
    if (date) {
      ...
      datestring += '-';
      datestring += date.toLocaleString(locale, { month: "short" });
      datestring += '-';
      ...
      datestring += ((date.getTimezoneOffset() > 0) ? ' -' : ' +' );
      datestring += ('0' + (-date.getTimezoneOffset() / 60)).slice(-2);
      datestring += ('0' + (-date.getTimezoneOffset() % 60)).slice(-2);
      datestring += '"';
    }
    
    this._send("APPEND " + this._escapeString(this._selectedMailbox.path) + (flags.length ? " (" + flags.join(" ")+")":"") + datestring + " {" + message.length+"}", (function(status, data){
        this._literalHandler = null;
        if(status == "OK"){
